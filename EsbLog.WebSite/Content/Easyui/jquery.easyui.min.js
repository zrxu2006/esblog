...
//2016/1/11添加
if (_8f.iconCls) {
	if (_8f.iconCls !== "icon-ok" && _8f.iconCls !== "icon-cancel" && _8f.iconCls !== "icon-add" && _8f.iconCls !== "icon-save" && _8f.iconCls !== "icon-remove" && _8f.iconCls !== "icon-undo" && _8f.iconCls !== "icon-allot" && _8f.iconCls !== "icon-edit" && _8f.iconCls !== "icon-search" && _8f.iconCls !== "icon-pull-in")
		_90 = $("<span class=\"l-btn-left\"></span>").appendTo(t);
	else
		_90 = $("<span class=\"l-btn-left-noicon\"></span>").appendTo(t);
} else {
	_90 = $("<span class=\"l-btn-left-noicon\"></span>").appendTo(t);
}
//添加结束
...
if(_8f.iconCls) {
//2016/1/11添加代码,去除icon-ok和icon-cancel
if(_8f.iconCls!=="icon-ok"&&_8f.iconCls!=="icon-cancel"&&_8f.iconCls!=="icon-add"&&_8f.iconCls!=="icon-save"&&_8f.iconCls!=="icon-remove"&&_8f.iconCls!=="icon-undo"&&_8f.iconCls!=="icon-allot"&&_8f.iconCls!=="icon-edit"&&_8f.iconCls!=="icon-search"&&_8f.iconCls!=="icon-pull-in")
{
...
//}
//}
//#region 修改tree如果onlyLeafCheck为true实现1.只有点击树叶才被选中(之前点击树枝也可以选择)2.点击整个节点都可以展开/收缩(之前只有点击三角图标才可以展开/收缩)
if(!_da.onlyLeafCheck){
if(tt.hasClass("tree-hit")){
_144(_d9,_dd[0]);
return false;
}else{
...
//原代码 cc.push("<span class=\"tree-checkbox tree-checkbox0\"></span>");
//#region 添加combotree值为空时去掉checkbox
	if (item.id !== "") {
...
//原代码 cc.push("<span class=\"tree-title\">"+opts.formatter.call(_1c3,item)+"</span>");
//#region 添加combotree值为空时灰色显示文本
if (item.id === "") {
	cc.push("<span class=\"tree-title\" style=\"color:#aaa\">" + opts.formatter.call(_1c3, item) + "</span>");
} else {
...
tip.css({left:pos.left,top:pos.top,zIndex:(opts.zIndex!=undefined?opts.zIndex:($.fn.window?($.fn.window.defaults.zIndex++)+110000:""))});
...
if (typeof (_240) == "undefined")return false;
...
//#region when textbox's editable=false or readonly,do not focus to the textbox
$(_49c).blur();
$(_49c).focus(function() {$(this).blur();}); 
//#endregion
...
//添加styler增加title功能---------
data.push("<td class=\""+cls+"\" abbr=\""+s+"\" style=\""+_539+"\""+_5391+">"+d+"</td>");
//-----------
}
data.push("</tr>");
}
...
//Èç¹ûÎÞÉèÖÃtoolbar²ÎÊýÊ±ÒÆ³ýdatagrid-toolbar
$("div.datagrid-toolbar",_5cf).remove();
}
...
if(opts.idField){
//#region 所有勾选项目排序 2016-03-04
if(_662.checkedRows.length>1) {
var rows = opts.finder.getRows(_661);
_662.checkedRows.sort(function(a,b){return (function() {for (var i = 0, length = rows.length; i<length; i++) {if (rows[i] == a) {return i;}}return 0;})()-(function() {for (var i = 0, length = rows.length; i<length; i++) {if (rows[i] == b) {return i;}}return 0;})();});
}
//#endregion
...
var tr=opts.finder.getTr(_6e6,(_6e7.index!=undefined?_6e7.index:_6e7.id));
if(!tr.length){
return;
}
var row=opts.finder.getRow(_6e6,tr);
var _64c=row[_6e7.field];
var td=tr.find("td[field=\""+_6e7.field+"\"]");
td.attr("rowspan",_6e7.rowspan).attr("colspan",_6e7.colspan);
//合并单元格优化
//td.addClass("datagrid-td-merged");
for(var i=1;i<_6e7.colspan;i++){
td=td.next();
td.hide();
...
var row=opts.finder.getRow(_6e6,tr);
var td=tr.find("td[field=\""+_6e7.field+"\"]").hide();
row[td.attr("field")]=_64c;
for(var j=1;j<_6e7.colspan;j++){
td=td.next();
td.hide();
row[td.attr("field")]=_64c;
}
...
(function($){
$(function(){
$(document).unbind(".combo").bind("mousedown.combo mousewheel.combo DOMMouseScroll.combo",function(e){
...
var _91e=_91d.panel;
if(!opts.editable){
//#region when combobox's editable=false,do not focus to the textbox
//$(_91c).combo("textbox").blur();
//$(_91c).combo("textbox").focus(function() {$(this).blur();}); 
//#endregion
_916(_91c);
}else{
...
var _968=$(_95c).combo("panel");
var _95e=$(_95c).combo("getValues");
if($.inArray(_95d+"",_95e)==-1){
if(opts.multiple){
//#region 多选时value为空关闭选项框并清空选项
//sourcecode:_95e.push(_95d);
	if (_95d === "") {
		$(_95c).combobox("hidePanel");
		$(_95c).combo("clear");
...
var v=_966[i];
var s=v;
//#region combobox的setvalue方法不让空值变选中
if(v!=""){
...
//#region comboboxÌí¼ÓÑ¡ÖÐ¿ÕvalueÇå¿Õtext´úÂë
	s = row[opts.valueField] === "" ? "" : row[opts.textField];
//#endregion
}
//原始代码
//    vv.push(v);
//    ss.push(s);
//#region combobox多选时value为空不setvalue
if (v != ""||!opts.multiple) {
	vv.push(v);
	ss.push(s);
} 
...
//原代码dd.push("<div id=\""+(_96c.itemIdPrefix+"_"+i)+"\" class=\""+cls+"\">");
//#region when combobox's value is empty.set the text gray
if(v===""){
	dd.push("<div id=\""+(_96c.itemIdPrefix+"_"+i)+"\" class=\""+cls+"\" style=\"color:#aaa\">");
}
else{
	dd.push("<div id=\""+(_96c.itemIdPrefix+"_"+i)+"\" class=\""+cls+"\">");
}
//#endregion
dd.push(opts.formatter?opts.formatter.call(_96a,row):v===""?"<span style=\"color:#aaa\">"+s+"</span>":s);
dd.push("</div>");
...
if(opts.multiple){
//#region combotree多选时移除选中样式
$(_99f).combo("panel").find("div.tree-node-selected").removeClass("tree-node-selected");
//#endregion
$(this).tree(node.checked?"uncheck":"check",node.target);
//#region combotree多选时选择空时清空值
if (node.id == "") {
$(_99f).combotree("setValues","");
$(_99f).combo("panel").find("div.tree-node-selected").removeClass("tree-node-selected");
$(_99f).combo("hidePanel");
}
 //#endregion
}else{
//#region combotree选中时过滤树节点的值
//if (opts.onlyLeafCheck && !$(this).tree('isLeaf', node.target)) {return;}
 //#endregion
$(_99f).combo("hidePanel");
}
_9a5(_99f);
...
//#region combotree选中时过滤树节点的值
if(!$(this).tree('isLeaf', _9a8[i].target)){continue;}
 //#endregion
vv.push(_9a8[i].id);
ss.push(_9a8[i].text);
}
...
//if (opts.onlyLeafCheck&&!$(this).tree('isLeaf', node.target)){return;}
 //#endregion
vv.push(node.id);
 //#region when combotree's value is empty.set the text empty
 //原代码ss.push(node.text);
ss.push(node.id===""?"":node.text)
 //#endregion
}
}
